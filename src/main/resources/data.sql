...
insert into asignatura (id, nombre, descripcion, curso)
	select 3, 'Historia', 'Asignatura de Historia', 3 from dual where not exists (select 1 from asignatura where id = 3);
...
insert into usuario (username, nombre, password, rol_id)
	select 'admin', 'Administrador', '$2a$10$5xOe75pbLcAjp0TbVWaluunrSshgYdH82YNwGd.b0Os4hAWbIEkry', 1 from dual where not exists (select 1 from usuario where username = 'admin');

insert into usuario (username, nombre, password, rol_id)
	select 'consulta', 'Consultador', '$2a$10$5xOe75pbLcAjp0TbVWaluunrSshgYdH82YNwGd.b0Os4hAWbIEkry', 2 from dual where not exists (select 1 from usuario where username = 'consulta');
